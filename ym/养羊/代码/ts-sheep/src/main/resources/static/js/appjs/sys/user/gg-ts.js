/*this file is used for the ts project, it is a unique js file*/
$(function(){
	//初始化icheck插件
	$(".i-checks").iCheck({
	...
	$(".gg-faeye").on('mouseover',this,function(){
		
		$(this).parent('div').children('input').prop('type', 'text')

	});
	$(".gg-faeye").on('mouseout',this,function(){
		
		$(this).parent('div').children('input').prop('type', 'password')
      ...
      if (this.support.datauri) {
        ...
        if (files.length > 0) {
          ...
          if (this.isImageFile(file)) {
            ...
            console.log(this.url);
            $('#head_row div').removeClass('pace-inactive')
            this.startCropper();
          }
        }
      } else {
    ...
    submitDone: function (data) {
      ...
      if ($.isPlainObject(data) && data.code === 0) {
        if (data.url) {
          ...
          $('#head_row').addClass('pace-inactive')//添加隐藏样式
        } else if (data.msg) {
