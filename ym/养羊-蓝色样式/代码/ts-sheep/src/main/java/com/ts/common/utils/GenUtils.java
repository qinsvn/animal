...
import com.ts.common.domain.ColumnDO;
import com.ts.common.domain.TableDO;
...
public class GenUtils {
    ...
    public static void generatorCode(Map<String, String> table,String className,String model, List<Map<String, String>> conditionColumns,List<Map<String, String>> resultColumns, List<Map<String, String>> allColumns, ZipOutputStream zip) {
        //配置信息
        ...
        if (StringUtils.isEmpty(className)) {
        	className = tableToJava(tableDO.getTableName(), config.getString("tablePrefix"), config.getString("autoRemovePre"));
		}
        String packageName = null;
        if (StringUtils.isEmpty(model)) {
        	packageName = config.getString("package").concat(".").concat(config.getString("model"));
		}else{
			packageName = config.getString("package").concat(".").concat(model);
		}
        ...
        for (Map<String, String> column : allColumns) {
        ...
        //查询条件列信息
        List<ColumnDO> conditionList = new ArrayList<>();
        for (Map<String, String> column : conditionColumns) {
            ColumnDO columnDO = new ColumnDO();
            columnDO.setColumnName(column.get("columnName"));
            columnDO.setDataType(column.get("dataType"));
            columnDO.setComments(column.get("columnComment"));
            ...
            conditionList.add(columnDO);
        }
        tableDO.setConditionColumns(conditionList);

        
        //查询结果列信息
        List<ColumnDO> resultList = new ArrayList<>();
        for (Map<String, String> column : resultColumns) {
            ColumnDO columnDO = new ColumnDO();
            columnDO.setColumnName(column.get("columnName"));
            columnDO.setDataType(column.get("dataType"));
            columnDO.setComments(column.get("columnComment"));
            ...
            resultList.add(columnDO);
        }
        tableDO.setResultColumns(resultList);
      //结束列处理------------------------------------------------------------------------------------------------------------------------------------------
        ...
        map.put("resultColumns", tableDO.getResultColumns());
        map.put("conditionColumns", tableDO.getConditionColumns());
    ...
    public static String tableToJava(String tableName, String tablePrefix, String autoRemovePre) {
        if (CommonConstant.AUTO_REOMVE_PRE.equals(autoRemovePre)) {
     ...
     * 获取文件名
     */
    public static String getFileName(String template, String classname, String className, String pakageName) {
        String classPath = "main" + File.separator + "java" + File.separator;
        String modulesName=pakageName.substring(pakageName.lastIndexOf(".") + 1);
        if (StringUtils.isNotBlank(pakageName)) {
        	  classPath += pakageName.replace(".", File.separator) + File.separator;
        }
        
        if (template.contains("domain.java.vm")) {
            return classPath + "domain" + File.separator + className + "DO.java";
        }

        if (template.contains("Dao.java.vm")) {
            return classPath + "dao" + File.separator + className + "Dao.java";
        }
...
//		}
        ...
        if (template.contains("Mapper.xml.vm")) {
            return "main" + File.separator + "resources" + File.separator + "mybatis" + File.separator + modulesName + File.separator + className + "Mapper.xml";
        }
