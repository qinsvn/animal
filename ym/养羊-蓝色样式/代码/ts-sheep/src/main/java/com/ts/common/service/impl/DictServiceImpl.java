...
import com.ts.common.dao.DictDao;
...
public class DictServiceImpl implements DictService {
    ...
    public int save(DictDO dict) {
        int ret = dictDao.save(dict);
        SystemUtil.setDicts(dicts());
        return ret;
    }

    @Override
    public int update(DictDO dict) {
        int ret = dictDao.update(dict);
        SystemUtil.setDicts(dicts());
        return ret;
    }
    ...
    public int batchRemove(Long[] ids) {
        int ret = dictDao.batchRemove(ids);
        SystemUtil.setDicts(dicts());
        return ret;
    }
    ...
    @Override
    public Map<Object, Map<Object, DictDO>> dicts() {
        Map<Object, Map<Object, DictDO>> ret = new HashMap<Object, Map<Object, DictDO>>();
        List<DictDO> list = dictDao.list(null);
        for (DictDO dictDO : list) {
            Map<Object, DictDO> typeMap = ret.get(dictDO.getType());
            if (typeMap == null) {
                typeMap = new HashMap<Object, DictDO>();
                ret.put(dictDO.getType(), typeMap);
            }
            typeMap.put(dictDO.getValue(), dictDO);
        }
        return ret;
    }
}
