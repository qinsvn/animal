...
function ajaxRequest(_url, _params, _callback, _callbackparams, _datatype,
		_contenttype, _method, _async) {
	...
	_contenttype = _contenttype ? _contenttype : 'application/json';
	$.ajax({
		url : _url,
		data : _params,
		dataType : _datatype,
		...
		success : function(data) {
			if(data.code=="A103"){
				top.location.href=window.top.serverInfo.base+"/login.html";
				return;
			}
			_callback(data, _callbackparams);
		},
		failure : function(data) {
			alert(data);
		}
...
function formatDate(timestamp) { 
	try{
		...
		return year+"-"+(month<9?('0'+month):month)+"-"+(date<9?('0'+date):date)+" "+(hour<9?('0'+hour):hour)+":"+(minute<9?('0'+minute):minute)+":"+(second<9?('0'+second):second); 
	}catch(e){
		return '-';
	}
}
