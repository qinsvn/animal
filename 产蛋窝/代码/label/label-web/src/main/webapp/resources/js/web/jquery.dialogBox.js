...
//The following is encapsulated by kuno
function set_message(_id,title,content){
	$(".dialog-box").remove();
	var diolog='<div id="auto-close-dialogBox"></div>';
	$("body").append(diolog);
	$('#'+_id).dialogBox({
		hasClose: true,
		autoHide: true,
...
function defind_text(_id,title,content,confirm,cancel){
	$('#'+_id).dialogBox({
		hasClose: true,
		autoHide: false,
		title: title,
		...
		hasBtn:true,
		confirmValue: "确定",
		confirm: confirm, //点击确定后回调函数
		cancelValue: "取消",  //取消按钮文字内容
		cancel:cancel,
	});
}
function defind_text_one(_id,title,content,confirm){
	$('#'+_id).dialogBox({
		hasClose: true,
		autoHide: false,
		title: title,
...
//取消事件
function __cancel(){
   $('#auto-close-dialogBox').attr("display",'none');
}