...
$(function(){
	...
	$("#del_company").click(function(){
		...
		if(delIds.length == 0){
			set_message("auto-close-dialogBox","提示","请选择需要删除的养殖户");
			return;
		}
		defind_text("auto-close-dialogBox","提示","确定删除选中的养殖户吗？",deleteCompanys,'');
...
//    	});
//    	if(comIds!=''){
//    		defind_text("auto-close-dialogBox","提示","确认要解除养殖户下的用户吗？",function(){
...
//			}, __cancel);
//    	}else{
//    		set_message("auto-close-dialogBox","提示",'请选择一个养殖户!');
//    		return ;
//    	}
//    });
    //end by jolley 
...
//			}
//		}
		if(!checkCZN("edit_name","养殖户名称",20)){
			return;
		}
		if($.trim(domain) != ""){
			var domainreg = /^[a-zA-Z_0-9\-\u4E00-\u9FA5]+$/;
			if(!domainreg.test($.trim(domain))){
				set_message("auto-close-dialogBox",'提示',"养殖范围只能输入中英文与数字");
				return;
			}
		}
		if($.trim(phone) == ""){
	...
	try
	{
		...
		if(json.code==0)
		{
			...
			ajaxPostFunctionAsytrue($("#basePath").val() + '/web/company/inserts',JSON.stringify(params),function(data){
				if(data.code==0){
					set_message("auto-close-dialogBox","提示","养殖户信息批量导入，成功 " + data.data.rows + " 行，失败 " + data.data.errRows + " 行");
					loadData(1);
				}else{
					set_message("auto-close-dialogBox","提示","养殖户导入失败！");
				}
			},null,"application/json",null)
		} 
	}
